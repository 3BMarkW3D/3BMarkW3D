...
async def get_counter(ctx, *, defense_name: str):
    ...
    if defense_key in playbook:
        ...
        embed = discord.Embed(
            title=f"📋 Offensive Coordinator Report",
            description=f"**Defense:** {strategy['name'].title()}",
            color=discord.Color.green(),
            timestamp=datetime.datetime.now()
        )
        
        embed.add_field(name="🎯 Top Counters", value=strategy['counters'], inline=False)
        embed.add_field(name="💡 Coach's Notes", value=strategy['notes'], inline=False)
        ...
        if matches:
            ...
            await ctx.send(f"❌ I don't have an exact match for **'{defense_name}'**, but I found these similar setups: **{suggestions}**.")
        else:
            await ctx.send(f"❌ Unrecognized defense: **'{defense_name}'**. Use `!add_play` to add it to the playbook!")
...
async def add_play(ctx, *, content: str):
    ...
    try:
        ...
        if len(parts) < 2:
            await ctx.send("⚠️ Format error! Please use: `!add_play Defense Name | Counter Plays | Notes`")
            return
        ...
        await ctx.send(f"✅ **{defense_name}** has been added to the playbook! Good work, Coach.")

    except Exception as e:
        await ctx.send(f"⚠️ Error adding play: {e}")
...
async def delete_play(ctx, *, defense_name: str):
    ...
    if defense_key in playbook:
        ...
        save_playbook(playbook)
        await ctx.send(f"🗑️ Removed **{defense_name}** from the playbook.")
    else:
        await ctx.send(f"❌ Could not find **{defense_name}**.")
